...
excelFiles.forEach((excelFile, index) => {
    ...
    data.forEach(row => {
        ...
        if (!wordFrequency[word].stories.includes(storyName)) {
            wordFrequency[word].stories.push(storyName);
        }
...
console.log(`\n总词汇数: ${totalWords}`);
console.log(`独特词汇: ${Object.keys(wordFrequency).length}`);
console.log(`故事数量: ${excelFiles.length}`);

// 生成HTML
console.log('\n开始生成HTML...');
const htmlContent = generateHTML(allVocabularies, excelFiles.length, Object.keys(wordFrequency).length);

// 保存文件
const outputPath = path.join(resultDir, 'CET4词汇学习表_完整版.html');
...
// 生成HTML函数
function generateHTML(vocabData, storyCount, uniqueWordCount) {
...
<head>
    ...
    <title>CET4词汇学习表 - ${vocabData.length}个词汇 - ${storyCount}个故事</title>
    <style>
...
<body>
    <div class="container">
        <h1>📚 CET4词汇学习表</h1>
        <div class="stats">
            <div class="stat-item"><div class="stat-number" id="total-words">${vocabData.length}</div><div class="stat-label">总词汇数</div></div>
            <div class="stat-item"><div class="stat-number" id="total-stories">${storyCount}</div><div class="stat-label">故事数量</div></div>
            <div class="stat-item"><div class="stat-number" id="unique-words">${uniqueWordCount}</div><div class="stat-label">独特词汇</div></div>
            <div class="stat-item"><div class="stat-number" id="filtered-words">${vocabData.length}</div><div class="stat-label">筛选结果</div></div>
        </div>
