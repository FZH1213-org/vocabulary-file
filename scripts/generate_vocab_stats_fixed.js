...
files.forEach(file => {
  ...
  // 查找复习版文件（更智能的查找方式）
  const reviewFiles = fs.readdirSync(resultDir).filter(f =>
    f.startsWith(num + '_') && f.endsWith('_复习版.html')
  );

  let reviewVocab = 0;
  if (reviewFiles.length > 0) {
    const reviewContent = fs.readFileSync(path.join(resultDir, reviewFiles[0]), 'utf-8');
...
// 统计不一致的数量
const inconsistent = storyData.filter(s => s.状态 === '不一致').length;
if (inconsistent > 0) {
  console.log(`发现 ${inconsistent} 个故事词汇数量不一致：`);
  storyData.filter(s => s.状态 === '不一致').forEach(s => {
    console.log(`  故事${s.序号}: 学习版${s.学习版词汇数}个, 复习版${s.复习版词汇数}个`);
  });
  console.log('');
}
