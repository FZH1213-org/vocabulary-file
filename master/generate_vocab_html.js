...
// 获取所有 Excel 文件（排除词汇表模板）
const files = fs.readdirSync(resultDir)
    .filter(file => file.endsWith('.xlsx'));

console.log(`找到 ${files.length} 个 Excel 文件`);

// HTML 模板 - 基于 词汇学习表_完整版.html 的样式
function generateHTML(storyName, words) {
    // 生成词汇数据
    const vocabularyData = words.map((w, i) => ({
        id: i + 1,
        word: w.word,
...
<body>
    <div class="container">
        <h1>📚 ${storyName}</h1>
        <div class="stats">
            <div class="stat-item"><div class="stat-number" id="total-words">${words.length}</div><div class="stat-label">总词汇数</div></div>
            <div class="stat-item"><div class="stat-number" id="filtered-words">${words.length}</div><div class="stat-label">筛选结果</div></div>
        </div>
...
function search(e){
    ...
    filteredData=vocabularyData.filter(v=>{
        return !q||v.word.toLowerCase().includes(q)||v.translation.includes(q);
    });
...
for (const file of files) {
    ...
    try {
        ...
        const words = data.map(row => ({
            ...
            chinese: row['中文含义'] || row['translation'] || row['chinese'] || ''
        })).filter(w => w.word && w.chinese);

        if (words.length === 0) {
            console.log(`跳过无词汇: ${file}`);
            continue;
        }

        // 提取故事名称
        // 01_职场女王_从实习生到总监.xlsx -> 职场女王：从实习生到总监
        const baseName = file.replace('.xlsx', '');
        const parts = baseName.split('_');
        let storyName = parts.slice(1).join('：') || baseName;

        // 生成 HTML
        const html = generateHTML(storyName, words);

        // 保存 HTML 文件
        // 01_职场女王_从实习生到总监.xlsx -> 01_职场女王_从实习生到总监_词汇表.html
        const htmlName = file.replace('.xlsx', '_词汇表.html');
        const htmlPath = path.join(resultDir, htmlName);

        fs.writeFileSync(htmlPath, html, 'utf-8');

        console.log(`${file} -> ${htmlName} (${words.length} 个词汇)`);
    } catch (err) {
        console.error(`处理失败: ${file}`, err.message);
    }
}

console.log(`\n完成! 共处理 ${files.length} 个 Excel 文件`);